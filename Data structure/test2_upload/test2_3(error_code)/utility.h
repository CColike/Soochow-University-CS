...
void instruction()
{
    std::cout<<"this is a program solving Infix Expression"<<std::endl;
    std::cout<<"you can input a Math Expression,ending with a '#'"<<std::endl;
    std::cout<<"such as :3*(2+1.1*2.)/2-3-(5.2+2.5)#"<<std::endl;
    std::cout<<"     or :-3*(-2+1.1*2)/2-(-3)-(5.2+2.5)#"<<std::endl;
    std::cout<<"     or :5aa.aa1aa+aa2aa.aa5aa#a#aa## "<<std::endl;
    std::cout<<"please input a math expression:";
}
...
void strengthen(std::string &s)//删除字符串中的其余character
{
    ...
    if(!flag_3)
    {
        std::cout<<"there is no '#' in your string,please check it"<<std::endl;
        exit(0);
    }
}
...
void string_error(int i)
{
    ...
    switch(i)
    {
        ...
        case 4:
            ...
            break;
        case 5:
            std::cout<<"improper mark matchong,please check it"<<std::endl;
            break;
    }
    exit(0);
}
